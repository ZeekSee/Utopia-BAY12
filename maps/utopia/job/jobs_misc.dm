...
/datum/job/assistant/equip(var/mob/living/carbon/human/H)
	if(H.mind.role_alt_title == "Ex-Researcher")
		var/datum/skillset/skillset = H.skillset
		skillset.skill_list[SKILL_BUREAUCRACY] = max(SKILL_BASIC, skillset.skill_list[SKILL_BUREAUCRACY])
		skillset.skill_list[SKILL_COMPUTER] = max(SKILL_BASIC, skillset.skill_list[SKILL_COMPUTER])
		skillset.skill_list[SKILL_DEVICES] = max(SKILL_BASIC, skillset.skill_list[SKILL_DEVICES])
		skillset.skill_list[SKILL_SCIENCE] = max(SKILL_ADEPT, skillset.skill_list[SKILL_SCIENCE])
		skillset.on_levels_change()
	. = ..()

/datum/job/assistant/get_description_blurb()
	return "Вы - утопиец. От Вас не ждут серьезного участия в раунде. \
