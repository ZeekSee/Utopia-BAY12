/datum/map/utopia
	shuttle_called_message = "Внимание всему персоналу: запущена процедура подготовки к подпространственному прыжку в центральный штаб GC. Расчётное время подготовки шаттла: %ETA%."
	shuttle_docked_message = "Внимание всему персоналу: подготовка к подпространственному прыжку завершена. Начата процедура безопасной активации генератора подпространства. Расчетное время до начала прыжка шаттла: %ETD%."
	shuttle_leaving_dock = "Внимание всему персоналу: прыжок начат, шаттл покинул станцию. Рассчётное время прибытия: %ETA%. Сохраняйте спокойствие и трезвость мышления."
	...
	emergency_shuttle_called_message = "Внимание всему персоналу: получен запрос аварийной эвакуации станции. Автоматическое подтверждение запроса через: %ETA%"
	...
	emergency_shuttle_leaving_dock = "Внимание всему персоналу: активация двигателей спасательных капсул. Расчётное время прибытия до центрального штаба GC: %ETA%"
	...
	meteor_detected_message = "Внимание всему персоналу, обнаружены метеориты прямо по курсу %STATION_NAME%. Настоятельно рекомендуется пройти в центр станции до выхода из опасной зоны."
	meteor_detected_sound = sound('sound/AI/torch/meteors.ogg', volume = 45)

	radiation_detected_message = "Зафиксировано повышение уровня космической радиации поблизости %STATION_NAME%. Всему персоналу настоятельно рекомендуется пройти в экранированные отсеки в технических тоннелях до ослабления космической радиоактивности."
	...
	unknown_biological_entities_message = "Неизвестные биологические существа обнаружены в космическом пространстве %STATION_NAME%. Настоятельно рекомендуется активировать режим сдерживания неизвестных форм жизни."

	unidentified_lifesigns_message = "Обнаружена неопознанная форма жизни на борту %STATION_NAME%. Настоятельно рекомендуется заблокировать все внешние шлюзы и запретить стыковку любым судам."
...
/datum/map/utopia/level_x_biohazard_announcement(var/bio_level)
	if(!isnum(bio_level))
	...
	switch(bio_level)
		...
		if(6)
			...
			crew = "Охранному"
		if(5)
			msg = "космические карпы. Настоятельно рекомендуется временно прервать всю деятельность вне станции"
			crew = "Охранному"
		if(4)
